...
public class Greedy_Controller extends Controller<MOVE>{
	...
	public MOVE getMove(Game game, long timeDue) {
		...
		if (CurrentMoves.isEmpty() == false){
			...
			// define target index, target is a random pill to the current node of pacman
			// current node index
...
//			}
			...
			CurrentMoves = greedy(currentIndex, targetIndex, lastMove, game);
			MOVE currentMove = CurrentMoves.remove(0);
			return currentMove;			
		}
	}

	public ArrayList<MOVE> greedy(int currentIndex, int targetIndex,
